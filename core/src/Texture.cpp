...
Texture::Texture(const std::string& FilePath, const unsigned int Unit)
{
    Load(FilePath, Unit);
}
...
void Texture::Load(const std::string& FilePath, const unsigned int Unit)
{
    ...
    else if (ChannelsCount == 3) Format = GL_RGB;
    else if (ChannelsCount == 4) Format = GL_RGBA;
    else
    {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, Format, Width, Height, 0, Format, GL_UNSIGNED_BYTE, ImageRawData);
    // Generates mipmaps
